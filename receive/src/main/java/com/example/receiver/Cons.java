package com.example.receiver;

public class Cons {

    final static String SOCEKTIO_SERVER = "http://15.164.61.120:8010/counsel";
    final static String API_SERVER = "http://15.164.61.120:8008";
    final static int WINDOW_WIDTH = 1600;
    final static int WINDOW_HEIGHT = 1000;
    final static int UPPER_PANEL_HEIGHT = 100;
    final static int CANVAS_WIDTH = 500;
    final static int JLIST_WIDTH = 800;
    final static int RIGHT_PANEL_WIDTH = 300;
    final static int LOGIN_PANEL_HEIGHT = 100;
    final static int CHAT_PANEL_HEIGHT = 60;
    final static int CUSTOMER_LIST_HEIGHT = 740;
}
